...
import (
	...
	// spearmint: Need pairing-friendly curves/library. TODO: Replace with bn256cgo for better performance.
...
// Generate the group structs (G1, G2, GT) for bn256
var g1 bn256.G1 // create G1 struct
var g2 bn256.G2 // create G2 struct
var gt bn256.GT // create GT struct

// Find generator of G2
var k = big.NewInt(1)
var h = g2.ScalarBaseMult(k) // get generator, h, of G2 (easier way to extract this?)

// Generated by the FW (hz sent to the client beforehand - abstracted away in this implementation).
var z, _ = rand.Int(prng, bn256.Order)
var hz = g2.ScalarBaseMult(z)
zneg := z.ModInverse(z, bn256.Order)
var chi, _ = rand.Int(prng, bn256.Order)
...
func keyExchangeSizeFromNamedGroup(group NamedGroup) (size int) {
	size = 0
	switch group {
	case BN256: // Added to spearmint.
		size = 64 // This case returns two key agreement values, each of size 32.
	case X25519:
		size = 32
	case P256:
		size = 65
...
func newKeyShare(group NamedGroup, isClient bool) (pub []byte, priv []byte, err error) {
	switch group {
	case P256, P384, P521:
		var x, y *big.Int
	...
	case BN256: // Shares are different for client and server
		var public, public1, public2 []byte
		err = nil // TODO: Add err checks.
		if (isClient == true) {
			x, _ := rand.Int(rand.Reader, bn256.Order)
			priv = x.Bytes()
			gx := g1.ScalarBaseMult(x)
			hzx := g2.ScalarMult(hz,x)
			...
			y, _ := rand.Int(rand.Reader, bn256.Order)
			priv = y.Bytes()
			gy := g1.ScalarBaseMult(y)
			c, _ := rand.Int(rand.Reader, bn256.Order)
			hprime := g2.ScalarBaseMult(c)

			public1 = gy.Marshal()
			public2 = hprime.Marshal()
			public = append(public1, public2...)
			pub = public[:]
			return
		}
...
func keyAgreement(group NamedGroup, pub []byte, priv []byte) ([]byte, error) {
	...
	case BN256: // This case returns two key agreement values.
		if len(pub) != 192 {
			return nil, fmt.Errorf("tls.keyagreement: Wrong public key size")
		}

		var public [192]byte
		var public1, ret []byte
		x := big.NewInt(0).SetBytes(priv) //Need to check this. Q: Get back from bytes for exponents?
		copy(public[:], pub)
		public1 = public[:64]
		X, _ := g1.Unmarshal(public1)
		agcs := g1.ScalarMult(X, x)
